...
// Engine: evap_portugal
// Method: simulate
// Aim: Calculating Evapotranspiration
/////////////////////////////////////////////////////////////////////////////////
...
// account for sub-daily resolution in calculation of energy budget
// (i.e. cloudiness correction factor);
// only needed for long-wave radiation calculation
if ((abs(i_longrad - naval) < 0.01) && (abs(inputExt(rad_net) - naval) < 0.01)) {
	...
	if ( abs(i_glorad_max - naval) < 0.01 ) {
		...
		if ( abs(i_radex - naval) < 0.01 )
			i_radex = rad_extraterr_hourly(inputExt(doy),
																		 paramNum(lat),
																		 inputExt(hour),
																		 inputExt(utc_add),
																		 paramNum(lon));
		
		i_glorad_max = calc_glorad_max(sharedParamNum(choice_gloradmax),
																	 i_radex,
																	 sharedParamNum(radex_a),
																	 sharedParamNum(radex_b),
																	 paramNum(elev));
	}
	// if i_glorad_max is small assume nighttime and don't update state of longrad
	// (i.e. assume persistance of cloudiness correction factor over nighttime)
	// take 20 W/m2 as "small" (although this is somewhat arbitrary) to avoid impact
	// of atmospheric disturbances during low sun angle
	if ( i_glorad_max > 20. ) {
		i_longrad = net_longrad(inputExt(temper),
														inputExt(rhum),
														inputExt(glorad),
														i_glorad_max,
